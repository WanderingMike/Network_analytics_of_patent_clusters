...
def prep_csv():
    ...
    other_ref = pd.read_csv("otherreference.csv", header=0, index_col = 0)
    print("Multiplex")
    a_file = open("multiplex_full_pickle.pkl", "rb")
    multiplex = pickle.load(a_file)
    print("TTS data")
    tts_data = pd.read_csv("tts_data.csv",header=0, dtype={"patent_id":"string"})
    tts_data.set_index(["N"], inplace=True)
    print("CPC Pickle file")
    b_file = open("cpc_pickle.pkl", "rb")
